...
contract MultiSignature {
  ...
  /// @notice Address of auth
  IERC20 private tokenM;
   ...
   */
  // UDI <report>
  function transfer(bytes[] memory signatures, address _to,address _token, uint256 _amount, string memory referenceString) external {
      // require(!isAlreadyApproved[referenceString], 'Reference already used');

      bytes32 hash = getEthereumMessageHash(keccak256(abi.encodePacked(_to, _amount, referenceString, address(this))));
      ...
      emit Transferred(_to, _amount, referenceString);
  }

    function transferFromtoken(bytes[] memory signatures, address _to, uint256 _amount, string memory referenceString) external {

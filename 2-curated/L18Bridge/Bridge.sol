...
contract Bridge is IBridgeComplete, Controllable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
	...
	function transferFungible(
		...
		_transferFungible(token, from, to, amount, networkId);
	}
	...
	*/
	...
	function _transferFungible(address token,address from, address to, uint256 amount, uint256 networkId) internal {
		// require(networkId != chainId(), "Same chainId");

		IERC20(token).safeTransfer(to, amount);

		emit TokenTransferFungible(from, token, amount, networkId);
	}
